...
// file:      BlockMatcherAW1.cu
//
// summary:   block matcher a w 1 class
//
...
#include "CluTec.Cuda.ImgProc/Kernel.Algo.AdaptWnd_W32H16.h"
		...
		namespace BlockMatcherAW1
		{
			...
			namespace Kernel
			{
					...
					struct Constants
				{
					...
					using AlgoAW = Clu::Cuda::Kernel::AlgoAdaptWnd_W32H16<ThreadsPerBlockX, t_iSubPatchSizeX, t_iSubPatchSizeY>;
					using AlgoSum = Clu::Cuda::Kernel::AlgoArraySum<ThreadsPerBlockX, 4, AlgoAW::PatchSizeX, AlgoAW::PatchSizeY>;


					// the width of a base patch has to be a full number of words
					static const TIdx BasePatchSizeX = AlgoAW::PatchSizeX;
					static const TIdx BasePatchSizeY = AlgoAW::PatchSizeY;
					static const TIdx BasePatchElementCount = AlgoAW::EffectivePatchElementCount;

					static const TIdx BasePatchCountX = AlgoAW::PatchCountX;
					static const TIdx BasePatchCountY = AlgoAW::PatchCountY;

					static const TIdx TestBlockSizeX = AlgoAW::DataArraySizeX;
					static const TIdx TestBlockSizeY = AlgoAW::DataArraySizeY;

					static const TIdx SumCacheSizeX = AlgoAW::SumCacheSizeX;
					static const TIdx SumCacheSizeY = AlgoAW::SumCacheSizeY;

					static const TIdx ResultCountPerThread = AlgoAW::EvalLoopCount;
...
#undef PRINT
				...
				{
					return (float(iSimValue) / float(Const::BasePatchElementCount)) * (100.0f / float(Clu::NumericLimits<TSrcValue>::Max()));
				}
				...
				{
					return (TResult)(TResult(Clu::NumericLimits<TValue>::Max()) - abs(TResult(xValA) - TResult(xValB)));
				}
				...
				__device__ __forceinline__ static TResult EvalSim(TValue xValA, TValue xValB, SSimType_SignedProduct )
				{
					...
					fVal1 *= fVal2;
					fVal1 = fVal1 * float(Clu::NumericLimits<TValue>::Max());
				...
				template<typename Const, typename TSimType, typename TPixelSurf, typename TSimStore, typename TDisp, typename TCacheBasePatch
					, typename TCacheSum, typename TCacheTestPatch>
					__device__ void EvalSimAW(TSimStore* pxSimStore
						, TDisp* pxDisp
				...
				{
					using TSimValue = typename TCacheSum::TElement;
					using TData = typename TCacheBasePatch::TElement;

					Const::AlgoAW::EvalCache<Const::AlgoAW::EType::Two_OutOf_Four>(xCacheSumY,
						[&xCacheBasePatch, &xCacheTestPatch, iBlockX, iBlockY](TIdx iIdxX, TIdx iIdxY)
					{
						auto pixVal1 = xCacheBasePatch.At(iIdxX, iIdxY);
						auto pixVal2 = xCacheTestPatch.At(iBlockX + iIdxX, iBlockY + iIdxY);
						return EvalSim<TSimValue>(pixVal1.x, pixVal2.x, TSimType());
					},
						[&pxSimStore, &pxDisp, &iDispIdx, &uDispIdxInc](TSimValue& iValue, TIdx iResultIdx, TIdx iIdxX, TIdx iIdxY)
					{
						...
						//		, xSimStore.iPrev2, xSimStore.iPrev, xSimStore.iMax, xSimStore.iNext, xSimStore.iNext2, xSimStore.iLast, iValue);
						//}
				...
				template<typename Const, typename TSrcValue, typename TDisp, typename TCacheBasePatch, typename TCacheSum2>
				__device__ void EvalGrad(TDisp* pxDisp
					, TCacheBasePatch& xCacheBasePatch
					, TCacheSum2& xCacheSumY2
					, float fGradThresh)
				{
					using TSum2 = typename TCacheSum2::TElement;

					TSum2 piSum[Const::AlgoSum::ColGroupSizeX];

					Const::AlgoSum::SumCache(xCacheSumY2,
						[&](TIdx iIdxX, TIdx iIdxY)
					{

						if (xCacheBasePatch.IsInside(iIdxX, iIdxY))
						{
							return (TSum2)xCacheBasePatch.At(iIdxX, iIdxY).x;
						}
						else
						{
							return TSum2(0);
						...
						[&](TSum2& iValue, TIdx iRelX, TIdx iIdxX, TIdx iIdxY)
					{
						piSum[iRelX] = iValue;
					});

					Const::AlgoSum::SumCacheStore(xCacheSumY2,
						[&](TIdx iIdxX, TIdx iIdxY)
					{
						TSum2 iValue = 0;
						...
						if (xCacheBasePatch.IsInside(iIdxX, iIdxY))
						{
							iValue = (TSum2)xCacheBasePatch.At(iIdxX, iIdxY).x;
						}

						return (iValue * iValue);
					},
						[&](TSum2& iSum2, TIdx iRelX, TIdx iIdxX, TIdx iIdxY)
					{
						const float fMean = float(piSum[Const::AlgoSum::ColGroupSizeX - iRelX - 1]) / float(Const::BasePatchSizeX * Const::BasePatchSizeY);
						const float fMean2 = float(iSum2) / float(Const::BasePatchSizeX * Const::BasePatchSizeY);
						const float fStdDev = sqrtf(abs(fMean2 - fMean * fMean));// / float(Clu::NumericLimits<TSrcValue>::Max());

						//if (Debug::IsBlock(10, 10))
						//{
						//	printf("%d,%d (%d / %d): %g, %g, %g\n", iIdxX, iIdxY, iRelX, Const::AlgoSum::ColGroupSizeX, fMean, fMean2, fStdDev);
						//}

						return (TSum2)fStdDev;
						
						//TSum2 iX = iIdxX - iRelX;
						//TSum2 iY = iIdxY;
						//return ((iX << 16) | iY);
					});
					...
					for (TIdx iEvalIdx = 0; iEvalIdx < Const::ResultCountPerThread; ++iEvalIdx)
					{
						TIdx iIdxX, iIdxY;
						if (Const::AlgoAW::GetResultXY(iIdxX, iIdxY, iEvalIdx))
						{
							//TSum2 iPos = xCacheSumY2.At(iIdxX, iIdxY);
							//TSum2 iY = iPos & 0xFFFF;
							//TSum2 iX = iPos >> 16;
							//float fStdDev = 0.1;

							const float fStdDev = float(xCacheSumY2.At(iIdxX/* + Const::AlgoAW::SubPatchSizeX*/, iIdxY/* + Const::AlgoAW::SubPatchSizeY*/))
								/ float(Clu::NumericLimits<TSrcValue>::Max()) * 100.0f;

							//if (Debug::IsBlock(10, 10))
							//{
							//	//printf("(%d, %d) -> (%d, %d)\n", iIdxX, iIdxY, iX, iY);
							//	printf("%d,%d (%d): %g\n", iIdxX/* + Const::AlgoAW::SubPatchSizeX*/, iIdxY /*+ Const::AlgoAW::SubPatchSizeY*/, iEvalIdx, fStdDev);
							//}

							if (pxDisp[iEvalIdx] == TDisp(EDisparityId::Unknown) && fStdDev < fGradThresh)
							{
								pxDisp[iEvalIdx] = TDisp(EDisparityId::CannotEvaluate);
							}
						}
					}
				...
				////////////////////////////////////////////////////////////////////////////////////////////////////
				/// <summary>	Writes a disparity. </summary>
				///
				/// <typeparam name="t_eConfig"> 	Type of the configuration. </typeparam>
				/// <typeparam name="TDispTuple">	Type of the disp tuple. </typeparam>
				/// <typeparam name="TDisp">	 	Type of the disp. </typeparam>
				/// <typeparam name="TSrcValue"> 	Type of the source value. </typeparam>
				/// <typeparam name="TSimStore"> 	Type of the simulation store. </typeparam>
				/// <param name="pxSimStore">	[in,out] If non-null, the px simulation store. </param>
				/// <param name="pxDisp">	 	[in,out] If non-null, the px disp. </param>
				/// <param name="iBlockX">   	Zero-based index of the block x coordinate. </param>
				/// <param name="iBlockY">   	Zero-based index of the block y coordinate. </param>
				////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				__device__ __forceinline__ void WriteDisparity(TSimStore* pxSimStore, TDisp* pxDisp, TIdx iBlockX, TIdx iBlockY)
				{
					...
					{
						TIdx iIdxX, iIdxY;
						if (AlgoAW::GetResultXY(iIdxX, iIdxY, iResultIdx))
						{
							...
							if (c_surfDisp.IsInside(iIdxX + iBlockX + Const::BasePatchSizeX, iIdxY + iBlockY + Const::BasePatchSizeY))
							{
								...
								else
								{
...

									{
										...
										//		, xSimStore.iPrev2, xSimStore.iPrev, xSimStore.iMax, xSimStore.iNext, xSimStore.iNext2, xSimStore.iLast
				...
				__global__ void Matcher()
				{
					...
					using TCacheSum = Clu::Cuda::Kernel::CArrayCache_W16<TSimValue>;

					using TCacheSum2 = Clu::Cuda::Kernel::CArrayCache<TSum2
						, Const::AlgoSum::SumCacheSizeX, Const::AlgoSum::SumCacheSizeY
						, Const::WarpsPerBlockX, Const::WarpsPerBlockY, 8, 1>;
					...
					__shared__ TCacheSum2 xCacheSumY2;
...
#					pragma unroll
					...
					EvalGrad<Const, TComponent>(pxDisp, xCacheBasePatch, xCacheSumY2, c_xPars.fGradThresh);
					// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
					// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					for (TIdx iDispIdx = c_xPars.xDispConfig.Min(); iDispIdx < iMaxDispIdx; iDispIdx += Const::SubDispCount)
					{
						...
						if (c_surfImgR.Format().IsRectInside(iTestX, iTestY, Const::TestBlockSizeX + Const::SubDispCount - 1, Const::TestBlockSizeY))
						{
							...
							{
								EvalSimAW<Const, TSimType, TPixelSrc>(pxSimStore, pxDisp, xCacheBasePatch, xCacheSumY, xCacheTestPatch
				...
				__global__ void MatcherUseDisparity()
				{
					...
					using TSimValue = short;
					using TSimStore = SSimStore<TSimValue>; //ushort4;
					using TIdx = short;
