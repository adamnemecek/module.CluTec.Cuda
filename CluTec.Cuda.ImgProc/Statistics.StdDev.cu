...
// file:      Statistics.StdDev.cu
//
// summary:   statistics. standard development class
//
					...
					__global__ void FilterNoBorder(Clu::Cuda::_CDeviceSurface xImageTrg, Clu::Cuda::_CDeviceSurface xImageSrc)
					{
						...
						for (int iResultIdx = 0; iResultIdx < Const::ResultCountPerThread; ++iResultIdx)
						{
							...
							if (iX < iW && iY < iH)
							{
								...
								const TProd uStdDev = (TProd)(c_xPars.fScale * sqrtf((fMean2 - fMean * fMean)));
								TPixel pixResult;

								pixResult.r() = (TData)min(uStdDev, (TProd)Clu::NumericLimits<TData>::Max());
...
#undef _CLU_DO_PROCESS
					...
					, const Clu::Cuda::_CDeviceSurface& xImageIn)
				{
					...
					{
						throw CLU_EXCEPTION("Given output image has different format than input image");
					}
