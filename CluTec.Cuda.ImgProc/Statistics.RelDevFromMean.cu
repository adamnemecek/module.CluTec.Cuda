...
// file:      Statistics.RelDevFromMean.cu
//
// summary:   statistics. relative development from mean class
//
...
#include "Statistics.RelDevFromMean.h"
...
#undef PRINT
					...
					__global__ void FilterNoBorder(Clu::Cuda::_CDeviceSurface xImageTrg, Clu::Cuda::_CDeviceSurface xImageSrc)
					{
						...
						// //////////////////////////////////////////////////////////////////////////////////////////////
							[&piSum, &xImageSrc, &iCtrX, &iCtrY, &iW, &iH](TSum& iValue, int iResultIdx, int iIdxX, int iIdxY)
						{
							const int iX = iCtrX + iIdxX + iResultIdx;
							const int iY = iCtrY + iIdxY;

							piSum[iResultIdx] = iValue * TSum(iX < iW && iY < iH);
						});
						// //////////////////////////////////////////////////////////////////////////////////////////////
						// //////////////////////////////////////////////////////////////////////////////////////////////
							...
							[&xImageSrc, &iPatchX, &iPatchY, &iW, &iH](int iIdxX, int iIdxY)
						{
							...
							if (iX < iW && iY < iH)
							{
								TSum iValue = (TSum)xImageSrc.Read2D<TData>(iX, iY);
							...
							// //////////////////////////////////////////////////////////////////////////////////////////////
							[&piSum, &xImageSrc, &xImageTrg, &iCtrX, &iCtrY, &iW, &iH](TSum& iValue, int iResultIdx, int iIdxX, int iIdxY)
						{
							const int iX = iCtrX + iIdxX + iResultIdx;
							const int iY = iCtrY + iIdxY;
							...
							const float fMean = float(piSum[iResultIdx]) / float(Const::BasePatchElementCount);
							const float fMean2 = float(iValue) / float(Const::BasePatchElementCount);

							const float fCenter = (float)xImageSrc.Read2D<TData>(iX, iY);

							const float fStdDev = max(c_xPars.fStdDevMin, sqrtf(fMean2 - fMean * fMean));

							const float fResult = max(-c_xPars.fDevRatioMax, min(c_xPars.fDevRatioMax, (fCenter - fMean) / fStdDev)) / c_xPars.fDevRatioMax;

							const TSum iResult = TSum(floor(((fResult + 1.0f) / 2.0f) * float(Clu::NumericLimits<TData>::Max())));
							const TData uValue = (TData)min(max(iResult, 0), (TSum)Clu::NumericLimits<TData>::Max());
